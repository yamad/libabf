...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	s = abf.FileInfo.DataSection.llNumEntries;
	if (abf.FileInfo.nDataFormat == ABF_INTEGERDATA)
		t = mxINT16_CLASS;
   	else if (abf.FileInfo.nDataFormat == ABF_FLOATDATA)
   		t = mxSINGLE_CLASS;
   	else {
   		abf.Close();
   		mexErrMsgTxt("Unknown type of data!");
   	}

	a = mxCreateNumericMatrix(1, s, t, mxREAL);
	p = mxGetPr(a);
	ret = abf.ReadData(p);
	abf.Close();
