...
private:    // Member variables.
   ...
   HANDLE m_hfSynchFile;                      // Handle to temporary file.
...
inline BOOL CSynch::Read(LPVOID lpBuf, DWORD dwFirstEntry, DWORD dwEntriesToRead)
{
   ...
   DWORD dwCurrentPos = SetFilePointer( m_hfSynchFile, 0, NULL, FILE_CURRENT );

   if (dwCurrentPos == INVALID_SEEK_VALUE)
      return FALSE;

   SetFilePointer( m_hfSynchFile, dwFirstEntry * sizeof(Synch), NULL, FILE_BEGIN );

   DWORD dwBytesRead = 0;
   BOOL bOK = ReadFile(m_hfSynchFile, lpBuf, dwBytesToRead, &dwBytesRead, NULL);

   // Restore the original file.
   SetFilePointer( m_hfSynchFile, dwCurrentPos, NULL, FILE_BEGIN );
