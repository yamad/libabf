...
#include "\AxonDev\Comp\common\ArrayPtr.hpp"   // Smart array pointer template class.
#include "\AxonDev\Comp\common\FileReadCache.hpp"
#include "\AxonDev\Comp\AxoUtils32\AxoUtils32.h"     // for AXU_* functions
#include "\Axondev\Comp\Common\crc.h"
...
static char *GetTagComment(ABFTag *pTag)
{
   ...
   for (UINT i=0; i<ABF_TAGCOMMENTLEN; i++)
      if (*ps++!=' ')
         break;
...
BOOL WINAPI ABF_BuildErrorText(int nErrorNum, const char *szFileName, char *sTxtBuf, UINT uMaxLen)
{
   ...
   char szTemplate[128];
   if (!LoadString(g_hInstance, nErrorNum, szTemplate, sizeof(szTemplate)))
   {
      char szErrorMsg[128];
      LoadString(g_hInstance, IDS_ENOMESSAGESTR, szTemplate, sizeof(szTemplate));
