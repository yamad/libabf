...
#define ABF_OEM_TO_ANSI(sz)   OemToCharBuff(sz, sz, sizeof(sz))
...
//
BOOL OLDH_GetFileVersion( HANDLE hFile, UINT *puFileType, float *pfFileVersion,
                          BOOL *pbMSBinFormat)
{
   TopOfFile TOF;
...
void OLDH_ABFtoABF15(ABFFileHeader *pFH)
{
   ...
   if ((pFH->fFileVersionNumber < 1.4) && 
       (strnicmp(pFH->sCreatorInfo, s_szAxEngine, strlen(s_szAxEngine))!=0))
   {
...
void OLDH_ABFtoCurrentVersion(ABFFileHeader *pFH)
{
   ...
   if( (pFH->fFileVersionNumber < ABF_V170) || (pFH->nFileType != ABF_ABFFILE) )
   {
      ...
      for ( uRegion = 0; uRegion < ABF_STATS_REGIONS; uRegion++ )
      {
...
//   
static BOOL ReadADCInfo(HANDLE hFile, float *Param, char *ADCLabel )
{
...
//   
static BOOL ReadADCNames(HANDLE hFile, char *ChannelName)
{
...
//   
static BOOL FetchexConvert( HANDLE hFile, ABFFileHeader *pFH, float *Param,
...
//   
static BOOL ReadCondit(HANDLE hFile, char *Condit)
{
...
//   
static int ClampexConvert( HANDLE hFile, ABFFileHeader *pFH, float *Param,
                           char *Comment, char *Label, int *pnError )
{
   char ADCLabel[ABF_ADCCOUNT*ABF_ADCUNITLEN];
   char DACLabel[ABF_DACCOUNT*ABF_DACUNITLEN];
...
//
BOOL OLDH_ReadOldHeader( HANDLE hFile, UINT uFileType, int bMSBinFormat,
