...
// 
// Usage:
//  WORD newcrc = CalculateCRC( oldcrc, bufadr, buflen )
// 
// Compile with -DTEST to generate program that prints CRC of stdin to stdout.
// Compile with -DMAKETAB to print values for awCRCtable to stdout

// the CRC polynomial. This is used by XMODEM (almost CCITT).
// If you change POLYNOMIAL, you must change awCRCtable[]'s initial value to that printed by InitCRCtab()
...
static const WORD awCRCtable[1<<BITS_PER_BYTE] =     // as calculated by InitCRCtab()
{
...
WORD CalculateCRC(WORD wInitCRC, const void *pvData, UINT uBytes)
{
   ...
   for (UINT i=0; i<uBytes; i++)
      crc = WORD((crc<<BITS_PER_BYTE) ^ awCRCtable[(crc>>(CRC_WIDTH-BITS_PER_BYTE)) ^ pData[i]]);
...
void InitCRCtab()
{
   ...
   for ( int b = 0; b <= (1<<BITS_PER_BYTE)-1; ++b )
   {
      int v = b << (CRC_WIDTH-BITS_PER_BYTE);
      for ( int i = BITS_PER_BYTE; --i >= 0; )
...
int main(int argc, char **argv)
{
   ...
   while( (nr = read( fd, buf, MAXBUF )) > 0 )
      crc = CalculateCRC( crc, (BYTE *)buf, nr );
