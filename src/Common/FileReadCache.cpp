...
//
BOOL CFileReadCache::Initialize(UINT uItemSize, UINT uCacheSize, HANDLE hFile, 
...
BOOL CFileReadCache::Get( UINT uFirstEntry, void *pvItems, UINT uEntries )
{
   ...
   while (uEntries)
   {
      ...
      memcpy(pItems, m_pItemCache + uStart * m_uItemSize, uBytes);
...
void *CFileReadCache::Get( UINT uEntry )
{
   ...
   return m_pItemCache + (uEntry - m_uCacheStart) * m_uItemSize;
}
...
BOOL CFileReadCache::LoadCache(UINT uEntry)
{
   ...
   return m_File.Read(m_pItemCache, m_uCacheCount * m_uItemSize);
}
