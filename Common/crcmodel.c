...
ulong cm_reflect (ulong v, int b)
{
 int   i;
 ulong t = v;
 for (i=0; i<b; i++)
...
void cm_nxt (p_cm_t p_cm, int ch)
{
 ...
 if (p_cm->cm_refin) uch = cm_reflect(uch,8);
...
{
 if (p_cm->cm_refot)
    return p_cm->cm_xorot ^ cm_reflect(p_cm->cm_reg,p_cm->cm_width);
 else
...
ulong cm_tab (p_cm_t p_cm, int index)
{
 ...
 if (p_cm->cm_refin) inbyte = cm_reflect(inbyte,8);
 ...
 for (i=0; i<8; i++)
    if (r & topbit)
       ...
       r<<=1;
 if (p_cm->cm_refin) r = cm_reflect(r,p_cm->cm_width);
